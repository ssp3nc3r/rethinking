...
transformed parameters{
    ...
    v_actor = (diag_pre_multiply(sigma_actor,L_Rho_actor) * z_actor)';  // TODO: LEARN ABOUT diag_pre_multiply
    a_actor   = v_actor[,1];
    bp_actor  = v_actor[,2];
    bpc_actor = v_actor[,3];
...
model{
    ...
    target += normal_lpdf(bp |0 , 1 );
    target += normal_lpdf(bpc | 0 , 1 );
    target += normal_lpdf(to_vector(z_block) | 0 , 1 );  // TODO: LEARN ABOUT to_vector
...
ggplot() + theme_tufte(base_family = 'sans') +
  ...
  geom_segment(data = Rho, aes(x = logpop, xend = logpop.adj, y = total_tools, yend = total_tools.adj, alpha = correlation)) +
